function [F0_vector_out,Xi_F_out,Xi_Z_out] = EPG_EC(N_in,alpha_in,SP,TR_in,T1_in,T2_in,is,maxstate,spoiled,varargin)
...
RF_phase_inc  = 150;  % Used linear RF phase increment ONLY if RF_phase_mode = +1  ;  A useful value is 50deg

% Switch between balanced and spoiled
if nargin > 8
    if spoiled
        unbalanced = 1;
    else
...
fa       = abs(SP)*alpha_in;                                % "alpha" is already a function in Matlab: not good
phi(1:numel(fa)) = -angle(SP);                               % Add complex slice profile
...
if ~isempty(varargin)
    % Add eddy currents
    ec = varargin{1};
    model_girf = @(phi1,ec)(ec(1) * cos(phi1) + ec(2) * sin(phi1));
    phi = model_girf(0:(N-1)*(pi/(((1+sqrt(5))/2))),ec);
end
